		...
		function sortClick(){
			if(!$(this).hasClass('active')){
				var oldVal = $(this).text(),
					...
					pTree = $(this).attr('data-product-tree'),
					pSort = $(this).attr('data-product-sort');
				$(this).addClass('active').html('<textarea id="tmpdata" />');
				$('#tmpdata').focus().val(oldVal).select().change(function(){
					CheckNumberFields($(this), 'int', oldVal);
					var t = $(this),newVal = $(this).val(), e = $(this).closest('.sort');
					t.remove();
					e.removeClass('active').html(newVal);
					if(newVal != pSort){
						$('.sort').unbind('click');
						$('.sort').bind('click', emptyClick);
						$.ajax({
							url: 'ajax.php',
							data: 'id='+pId+'&idtree='+pTree+'&oldSort='+pSort+'&newSort='+newVal+'&url='+url+'&action='+action+'&id_tree='+id_tree+'&page='+page+'&orderby='+orderby,
							type: "GET",
							error: function(error) {},
							success: function(result) {
								if(result != ''){
									$('.product_list tbody').html(result);
									$('.sort').bind('click',sortClick);
								}else{
					...
					if(eventObject.keyCode == 13){
						$(this).change();
						return false;
					}else if(eventObject.keyCode ==27){
						e = $(this).closest('.sort');
						$(this).remove();
						e.removeClass('active').html(oldVal);
						return false;
					}else{
						CheckNumberFields($(this), 'int', '');
					}
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="product_list">
			<thead>
				<tr>
					...
					<th style="width:82px; text-align: center; border:none;">Позиция</th>
					...
					<?php if($Config['section']['price']) echo '<th style="width:180px; text-align: center; border:none;">Цена</th>'; ?>
